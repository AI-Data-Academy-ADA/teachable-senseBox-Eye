...
This is a tutorial for training an image classification model using teachable machines and deploy it to senseBox Eye.
...
### 2. train
Open [https://teachablemachine.withgoogle.com/](https://teachablemachine.withgoogle.com/) in Google Chrome. Click on "Image Project". Select "Embedded image model". You should see the following:
![alt text](figures/blank_project.png)

Rename "Class 1" to a name of your choosing (for example, an object, a gesture, a shape, etc.). Click on "Add Image Samples: Device". Click on "Attempt to connect to device". Orient the camera to see the class that you choose. Click on "Hold to Record" to record some training images for this class.
...
Now open `deploy/senseBox-Eye-deploy.ino` in the Arduino IDE. Choose "sensebox Eye" as the board. If your image stream using the Processing IDE is still running, stop it and restart the senseBox Eye. Then Compile and Upload the deploy script.
